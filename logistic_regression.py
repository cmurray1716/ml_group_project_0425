...
#Huge class imbalance so adding undersampling
rus = RandomUnderSampler(random_state=54)
cluster = ClusterCentroids(random_state=54)
cnn = CondensedNearestNeighbour(random_state=54)
tl = TomekLinks()

trainx_v_resampled, trainy_resampled = rus.fit_resample(trainx_v, trainy)
#trainx_v_resampled, trainy_resampled = cnn.fit_resample(trainx_v, trainy)
#trainx_v_resampled, trainy_resampled = tl.fit_resample(trainx_v, trainy)
#trainx_v_resampled, trainy_resampled = cluster.fit_resample(trainx_v, trainy)
...
pipeline = Pipeline([('tfidf', TfidfVectorizer()), ("ran", RandomUnderSampler()), ('lgr', LogisticRegression(max_iter=10000))])

#Setting up parameters for hyperparameter tuning
solvers = ["newton-cg", "lbfgs", "sag", "saga"]
penalty = ['l2']
c_values = np.linspace(0.01, 100, num=20)
min_df = np.linspace(0.01, 0.5, num=20)
max_features = np.arange(1,1100)

grid = dict(lgr__solver = solvers, lgr__penalty = penalty, lgr__C = c_values, tfidf__min_df = min_df, tfidf__max_features = max_features)
